...
# Load the 'businesses.csv' file and save it as a DataFrame called 'businesses'
businesses = pd.read_csv('datasets/businesses.csv')

# Sort the 'businesses' DataFrame by 'business_start_date' in ascending order
...
# Load the 'countries.csv' file and save it as a DataFrame called 'countries'
countries = pd.read_csv('datasets/countries.csv')

# Merge sorted_businesses with countries and save the merged DataFrame as 'businesses_countries'
businesses_countries = sorted_businesses.merge(countries, on='country_code')

# Filter businesses_countries to create a new DataFrame 'north_america' for countries in North America
north_america = businesses_countries[businesses_countries['continent'] == 'North America']

# Display the first few rows of the 'north_america' DataFrame
...
# Assuming you have the 'businesses_countries' DataFrame from previous steps

# Create the 'continent' DataFrame with the minimum 'year_founded' for each continent
continent = businesses_countries.groupby('continent')['year_founded'].agg('min').reset_index()

# Set 'continent' as the index of the DataFrame
continent.set_index('continent', inplace=True)

# Rename the 'year_founded' column to match the requirement
continent.rename(columns={'year_founded': 'year_founded'}, inplace=True)

# Merge 'continent' with 'businesses_countries' on multiple columns: 'continent' and 'Oldest Year Founded'
merged_continent = pd.merge(continent, businesses_countries, on=['continent', 'year_founded'])

# Subset the merged DataFrame to include only the four columns: continent, country, business, and year_founded
subset_merged_continent = merged_continent[['continent', 'country', 'business', 'year_founded']]

# Display the first few rows of the 'subset_merged_continent' DataFrame
print(subset_merged_continent.head())
...
# Assuming you have the 'businesses' DataFrame and new_businesses.csv file

# Import new_businesses.csv
new_businesses = pd.read_csv("datasets/new_businesses.csv")

# Vertically stack businesses and new_businesses together
...
# Group by continent and calculate the count of missing countries in each continent
count_missing = new_missing_countries.groupby('continent').agg({'country': 'count'})

# Rename the column to 'count_missing'
...
# Import categories.csv and merge with businesses
...
count_business_cats = businesses_categories.groupby("category").agg({"business": "size"})
...
businesses_categories_countries = businesses.merge(categories, on='category_code', how='inner').merge(countries, on='country_code', how='inner')
