#perform hill climbing search on a given problem4
import math
class Node:
...
def hillClimbing():
    ...
    parentCost=math.sqrt((
        graph[goalState].heuristic[0] - graph[initialState].heuristic[0]
    ...
    while parentNode != goalState:
        ...
        for child in graph[parentNode].actions:
            if child[0] not in explored:
                childCost = math.sqrt((
                    graph[goalState].heuristic[0] - graph[child[0]].heuristic[0]
